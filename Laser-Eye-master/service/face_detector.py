...
class MxnetDetectionModel(BaseDetection):
    ...
    def detect(self, image, mode='nms'):
        ...
        # 寻找置信度最高的人脸检测框坐标
        # print(self._biggest_wrapper(detach))
        # print(self._nms_wrapper(detach))
        if len(detach) > 0:
            face_location = self._nms_wrapper(detach)
            for r in face_location:
                cv2.rectangle(image, (int(r[0]), int(r[1])),
                              (int(r[2]), int(r[3])), (0, 255, 0))
    ...
    def workflow_postprocess(self, outstream=None):
        for frame, out in self.read_queue:
            ...
            if outstream is None:
                ...
                outstream(frame)
                outstream(detach)
    def workflow_postprocess_detect(self,image):
        detach = self._retina_detach(image)
            # print(f'workflow_postprocess: {time.perf_counter() - st}')
        for res in self._nms_wrapper(detach):
                # self.margin_clip(res)
